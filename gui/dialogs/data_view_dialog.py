...
class DataViewDialog(QDialog):
    def __init__(self, parent=None, average_brightness=None, center_brightness_save=None, time_save=None):
        ...
        self.setup_ui()
         ...
         #加载图表数据
        self.load_plot_data(self.average_brightness, self.center_brightness_save, self.time_data)

        self.result_N = 0

        #为计算N单独开一个方法
        self.calculate_N(self.average_brightness, self.center_brightness_save)
    ...
    def calculate_N(self, average_brightness, center_brightness_save):
        if self.average_brightness is not None and self.center_brightness_save is not None:
            try:
                self.N = FigureN.figureN(average_brightness, center_brightness_save)
                self.result_N_value.setText(str(self.N))
            except Exception as e:
                log_debug(f"计算N值时发生错误: {e}")

    #可视化数据
    def load_plot_data(self, average_brightness, center_brightness_save, time_data):
        ...
        if average_brightness is not None:
            self.plot_widget.addLine(y=average_brightness, pen=pg.mkPen('b', width=2, style=Qt.PenStyle.DashLine), name='平均亮度')
